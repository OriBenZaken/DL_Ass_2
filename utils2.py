...
def get_word_embeddings_matrix():
    """
    returns numpy matrix of the words embedding from
    wordVectors.txt
    :return: words vectors numpy matrix
    """
...
def read_tagged_data(file_name, is_dev = False):
    ...
    tagged_sentences = []
    """
    The file format for the tagging data is: 
    each line contains a word SPACE tag pair. Blank lines are sentence boundaries.
    """
...
def read_not_tagged_data(file_name):
    ...
    sentences = []
    """
    The file format for the test data is: 
    each line contains a word. Blank lines are sentence boundaries.
    """
    with open(file_name) as file:
        content = file.readlines()
        sentence = []
